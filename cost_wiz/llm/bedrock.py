import boto3
from loguru import logger
from cost_wiz.config.settings import EnvSettings
from prompt import get_prompt
from models.claude_3_with_text import Claude3Wrapper


def get_text(env: EnvSettings, columns, data, instance, logger):
    client = boto3.client(service_name="bedrock-runtime",
                          aws_access_key_id=env.aws_access_key_id,
                          aws_secret_access_key=env.aws_secret_access_key,
                          aws_session_token=env.aws_session_token,
                          region_name=env.aws_region_name)
    wrapper = Claude3Wrapper(client, logger)
    prompt = get_prompt(columns=columns, data=data, instance=instance)
    return wrapper.invoke_claude_3_with_text(prompt)


env: EnvSettings = EnvSettings()
columns = f"""timestamp,Average_CPUUtilization,Maximum_CPUUtilization,Minimum_CPUUtilization"""
data = f"""
            timestamp,Average_CPUUtilization,Maximum_CPUUtilization,Minimum_CPUUtilization
            2024-03-10,0.2393241832,0.5084095948,0.2083472231
            2024-03-11,0.2198618488,0.2499333511,0.1999533442
            2024-03-12,0.2630860413,2.760357596,0.1999666722
            2024-03-13,0.2329128924,0.366568915,0.1917145953
        """
instance = "t3.nano"
gg = get_text(env=env, columns=columns, data=data, instance=instance, logger=logger)

